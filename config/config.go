...
func LoadConfig(path string) IConfig {
	...
	if err != nil {
		log.Fatal("load dotenv failed: %v", err)
	}
	return &config{
		app: &app{
			...
			port: func() int {
				...
				if err != nil {
					log.Fatal("parse APP_PORT failed: %v", err)
				}
				return p
			}(),
			...
			readTimeout: func() time.Duration {
				...
				if err != nil {
					log.Fatal("parse APP_READ_TIMEOUT failed: %v", err)
				}
			...
			writeTimeout: func() time.Duration {
				...
				if err != nil {
					log.Fatal("parse APP_WRITE_TIMEOUT failed: %v", err)
				}
				return time.Duration(int64(t) * int64(math.Pow10(9)))
			}(),
			bodyLimit: func() int {
				...
				if err != nil {
					log.Fatal("parse APP_BODY_LIMIT failed: %v", err)
				}
				return b
			}(),
			fileLimit: func() int {
				...
				if err != nil {
					log.Fatal("parse APP_FILE_LIMIT failed: %v", err)
				}
				return f
			}(),
		...
		jwt: &jwt{
			...
			secretKey: envMap["JWT_SECERT_KEY"],
...
func (a *app) Name() string                { return a.name }                                //name
func (a *app) Version() string             { return a.version }                             //version
...
func (d *db) Url() string {
	return fmt.Sprintf("host=%s port=%d user=%s password=%s sslmode=%s", d.host, d.port, d.username, d.password, d.sslMode)
}
...
type jwt struct {
	...
	secretKey        string
	apiKey           string
	accessExpiresAt  int //seconds since
